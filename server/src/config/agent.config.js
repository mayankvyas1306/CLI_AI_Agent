...
/**
 * Zod schema for structired application generation
 */
const ApplicationSchema = z.object({
  ...
  files: z
    .array(
      z.object({
        path: z.string().describe("Relative file patj (e.g src/App.jsx)"),
...
export async function generateApplication(
  ...
  try {
    ...
    printSystem(chalk.magenta("🤖 Generating structures output...\n"));

    const result = await generateObject({
      model: aiService.model,
      schema: ApplicationSchema,
      prompt: `Create a complete, profuction-ready application for: ${description}
